...
def main():
    """主函数"""
    print("="*80)
    print("AlphaX - 100只A股标的策略回测验证（真实数据）")
    ...
    # 获取数据
    stock_data = data_manager.get_multiple_stocks(
        symbols=all_symbols,
        start_date=start_date,
        end_date=end_date,
        interval="d",
    ...
    if results:
        ...
        result_file = f"data/backtest_results/100stocks_real_data_{timestamp}.json"
        ...
        print("回测总结")
        print(f"{'='*80}")
        print(f"回测标的: {len(stock_data)} 只A股（真实数据）")
