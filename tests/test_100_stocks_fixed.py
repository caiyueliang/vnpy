"""
100只A股标的策略回测验证（修复风控参数）

修复风控参数与策略仓位计算的冲突，确保策略能够正常交易
"""
...
def run_strategy_backtest(
    ...
    risk_limits = RiskLimits(
        max_single_position_pct=0.50,  # 单品种最大持仓比例 50%（放宽）
    ...
    position_config = PositionConfig(
        base_position=0.05,  # 基础仓位 5%（降低）
        max_position=0.15,   # 最大仓位 15%（降低）
        risk_per_trade=0.02,
    )
...
def main():
    """主函数"""
    print("="*80)
    print("AlphaX - 100只A股标的策略回测验证（修复风控参数）")
    ...
    strategies_config = [
        {
            ...
            "params": {
                ...
                "long_threshold": 0.2,  # 降低阈值，增加交易机会
                "short_threshold": -0.2,
                "exit_threshold": 0.05,
            }
        },
    ...
    if results:
        ...
        result_file = f"data/backtest_results/100stocks_fixed_{timestamp}.json"
