...
# 数据层导入
from data.realistic_data_generator import RealisticDataGenerator, get_all_symbols
...
def main():
    """主函数"""
    print("="*80)
    print("AlphaX - 100只A股标的策略回测验证（改进模拟数据）")
    ...
    from data.realistic_data_generator import STOCKS_100
    ...
    data_generator = RealisticDataGenerator(seed=42)  # 固定随机种子保证可复现
    ...
    if len(stock_data) < 50:
        print(f"\n警告: 仅生成到 {len(stock_data)} 只股票数据，数量不足")
        return
    ...
    if results:
        ...
        result_file = f"data/backtest_results/100stocks_improved_data_{timestamp}.json"
